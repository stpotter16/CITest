...
int solver(Heat* problem)
{
	...
	double new;
	double delta;
	if(strcmp((*problem).method, "Gauss-Seidal")){
		grvy_printf(GRVY_DEBUG, "Using solver method Gauss-Seidal");
...
int hdf5_output(Heat* problem)
{
	...
	dsp_data = H5Screate_simple(2, dimsdata, NULL);

	if((*problem).dimension==1){
		dst_data = H5Dcreate2(file, "Node X T", H5T_NATIVE_DOUBLE, dsp_data, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
		int i;
		double xcoord;
		double val;
		if((*problem).verification){
		...
		dst_data = H5Dcreate2(file, "Node X Y T", H5T_NATIVE_DOUBLE, dsp_data, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
		int i;
		int j;
		double xcoord;
		double ycoord;
		...
		for(i=0; i<(*problem).n; i++){
			...
			for(j=0; j<(*problem).n; j++){
				...
				data[i*(*problem).n + j][0] = node;
				data[i*(*problem).n + j][1] = xcoord;
				data[i*(*problem).n + j][2] = ycoord;
				data[i*(*problem).n + j][3] = val;
			}
		}
	}
	
	status = H5Dwrite(dst_data, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, data);
	status = H5Dclose(dst_data);
	status = H5Sclose(dsp_data);		
